...
export class TrackDownloadService {
  ...
  async download(track) {
    ...
    if (track.source === 'youtube') return provider.download(track);
    const resolved = await provider.resolveDownload(track);
    const filePath = temporaryPath(this.tempDir, extensionFromFilename(new URL(resolved.url).pathname, '.mp3'));
    try {
