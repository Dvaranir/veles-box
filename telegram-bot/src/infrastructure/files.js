...
export async function downloadResponse(response, destination) {
  ...
  await pipeline(Readable.fromWeb(response.body), (await fs.open(destination, 'w')).createWriteStream());
}

export async function downloadUrl(url, destination, { timeoutMs = 120_000, headers = {} } = {}) {
