...
export function parseSkySoundDownloadUrl(html) {
  const $ = cheerio.load(html);
  const url = $('.onesongblock-down.__adv_download').first().attr('href');
...
export class SkySoundProvider {
  ...
  async search(query) {
    const response = await fetch(skySoundSearchUrl(query), {
      ...
      headers: { 'user-agent': 'Mozilla/5.0 (compatible; VelesMusicBot/1.0)' },
    });
  ...
  async resolveDownload(track) {
    ...
    return {
      ...track,
      url: new URL(parseSkySoundDownloadUrl(await response.text()), track.detailUrl).toString(),
    };
  }
}
